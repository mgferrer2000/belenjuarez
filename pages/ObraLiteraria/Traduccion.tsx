...
import { TRANSLATION_WORKS } from './traduccionData';

const Traduccion: React.FC = () => {
    return (
        <div className="max-w-6xl mx-auto pb-24">
            <header className="mb-16 border-b border-gold/10 pb-10">
                ...
                <div className="max-w-3xl space-y-5 text-ink/70 font-light leading-relaxed text-lg text-justify">
                    <p>
                        Belen Juarez ha participado en la traduccion de dos libros y dossiers poeticos. Este apartado
                        reune esas colaboraciones en una pequena galeria editorial desde la que acceder a cada lectura bilingue.
                    </p>
                    <p>
                        Cada ficha abre una pagina de detalle donde los poemas se presentan cara a cara, con el texto original y su version traducida en paralelo.
                    </p>
                </div>
            </header>

            <section className="grid grid-cols-1 md:grid-cols-2 gap-12 lg:gap-16 items-start">
                {TRANSLATION_WORKS.map((work) => (
                    <article key={work.slug} className="group flex h-full flex-col">
                        <Link to={`/obra-literaria/traduccion/${work.slug}`} className="block mb-8">
                        ...
                        <div className="flex flex-1 flex-col space-y-4 px-1">
                            ...
                            <div className="min-h-[7.75rem] flex flex-col justify-start">
                                <Link to={`/obra-literaria/traduccion/${work.slug}`}>
                            ...
                            <div className="pt-3">
                                <Link
                                    to={`/obra-literaria/traduccion/${work.slug}`}
